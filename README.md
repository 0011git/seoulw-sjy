...
<summary>$\bf{\rm{\color{#0969DA}추가\ 설명:\ API\ URL}}$</summary>
  ...
  #### 2. 메인
  - 카테고리와 동일한 데이터를 사용, 1단위 요청 시 각각의 컨텐츠별로 묶어 **(이번주 공연, 공연 예정, 장르별)** 총 3그룹으로 나누어 API를 3번 요청
  - 카테고리 페이지의 API 요청 함수를 사용하면 24번의 요청이 필요해 **요청 횟수를 줄이기 위해 메인 전용 함수를 별도로 작성**함
